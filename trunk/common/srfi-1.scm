...
;*=====================================================================*/
;*    serrano/prgm/project/bigloo/contrib/srfi-1/srfi-1.bgl            */
;*    -------------------------------------------------------------    */
;*    Author      :  Manuel Serrano                                    */
;*    Creation    :  Wed Feb 24 21:27:38 1999                          */
;*    Last change :  Sat Nov  6 15:02:20 1999 (serrano)                */
;*    -------------------------------------------------------------    */
;*    The Bigloo module clause for the srfi-1 implementation           */
;*=====================================================================*/
;*-------------------------------------------------------------------*;
;*  Modified by Vladimir Tsichevski
;*-------------------------------------------------------------------*;
...
(module srfi-1
   ...
   (export (xcons::pair d a)
	   (make-list::pair-nil ::int #!optional elt)
	   ...
	   (tenth ::pair)
	   (car+cdr ::pair)
	   (take::pair-nil ::pair-nil ::int)
	   (take!::pair-nil ::pair-nil ::int)
	   (drop::pair-nil ::pair-nil ::int)
	   (drop!::pair-nil ::pair-nil ::int)
	   (take-right::pair-nil lis::pair-nil k::int)
	   (drop-right::pair-nil lis::pair-nil k::int)
	   (drop-right!::pair-nil lis::pair-nil k::int)
	   (split-at x k::int)
	   (split-at! x k::int)
	   (last ::pair-nil)
	   (unzip1::pair-nil lis::pair-nil)
	   (unzip2 ::pair-nil)
	   (unzip3 ::pair-nil)
	   (unzip4 ::pair-nil)
	   (unzip5 ::pair-nil)
	   (append-reverse rev-head::pair-nil tail)
	   (append-reverse! rev-head::pair-nil tail)
	   (concatenate::pair-nil lists::pair-nil)
	   (concatenate!::pair-nil  lists::pair-nil)
	   (count::int pred::procedure list1::pair-nil . lists)
	   (unfold-right p::procedure f::procedure g::procedure seed
			 #!optional
                          (tail::pair-nil '()))
	   (unfold p::procedure f::procedure g::procedure seed
			 #!optional tail-gen)
	   (fold kons::procedure knil lis1::pair-nil . lists)
	   (fold-right kons::procedure knil lis1::pair-nil . lists)
	   (pair-fold-right f::procedure zero lis1::pair-nil . lists)
	   (pair-fold f::procedure zero lis1::pair-nil . lists)
	   (reduce f::procedure ridentity lis::pair-nil)
	   (reduce-right f::procedure ridentity lis::pair-nil)
	   (append-map::pair-nil f::procedure lis1::pair-nil . lists)
	   (append-map!::pair-nil f::procedure lis1::pair-nil . lists)
	   (pair-for-each proc::procedure lis1::pair-nil . lists)
	   (map!::pair-nil f::procedure lis1::pair-nil . lists)
	   (filter-map::pair-nil f::procedure lis1::pair-nil . lists)
	   (map-in-order::pair-nil f::procedure lis1::pair-nil . lists)
	   (filter::pair-nil pred::procedure lis::pair-nil)
	   (filter!::pair-nil pred::procedure lis::pair-nil)
	   (partition pred::procedure lis::pair-nil)
	   (partition! pred::procedure lis::pair-nil)
	   (remove::pair-nil ::procedure lis::pair-nil)
	   (remove!::pair-nil ::procedure lis::pair-nil)
	   (delete::pair-nil x lis::pair-nil #!optional
                             (elt=::procedure equal?))
	   (delete!::pair-nil x lis::pair-nil #!optional
                              (elt=::procedure equal?))
	   (member x lis::pair-nil #!optional (elt=::procedure equal?))
	   (delete-duplicates::pair-nil lis::pair-nil #!optional (elt=::procedure equal?))
	   (delete-duplicates!::pair-nil lis::pair-nil #!optional (elt=::procedure equal?))
	   (assoc x lis::pair-nil #!optional (elt=::procedure equal?))
	   (alist-cons::pair key datum alist::pair-nil)
	   (alist-copy::pair-nil alist::pair-nil)
	   (alist-delete::pair-nil key alist::pair-nil #!optional (elt=::procedure equal?))
	   (alist-delete!::pair-nil key alist::pair-nil #!optional (elt=::procedure equal?))
	   (find pred::procedure list::pair-nil)
	   (find-tail pred::procedure lis::pair-nil)
	   (take-while::pair-nil pred::procedure lis::pair-nil)
	   (drop-while::pair-nil pred::procedure lis::pair-nil)
	   (take-while!::pair-nil pred::procedure lis::pair-nil)
	   (span pred::procedure lis::pair-nil)
	   (span! pred::procedure lis::pair-nil)
	   (break  pred::procedure lis::pair-nil)
	   (break! pred::procedure lis::pair-nil)
	   (any pred::procedure lis1::pair-nil . lists)
	   (every pred::procedure lis1::pair-nil . lists)
	   (list-index pred::procedure lis1::pair-nil . lists)
	   (lset= elt=::procedure . lists)
	   (lset<= elt=::procedure . lists)
	   (lset-adjoin::pair-nil =::procedure lis::pair-nil . elts)
	   (lset-union::pair-nil elt=::procedure . lists)
	   (lset-union!::pair-nil elt=::procedure . lists)
	   (lset-intersection::pair-nil elt=::procedure lis1::pair-nil . lists)
	   (lset-intersection!::pair-nil elt=::procedure lis1::pair-nil . lists)
	   (lset-difference::pair-nil elt=::procedure lis1::pair-nil . lists)
	   (lset-difference!::pair-nil elt=::procedure lis1::pair-nil . lists)
	   (lset-xor::pair-nil elt=::procedure . lists)
	   (lset-xor!::pair-nil elt=::procedure . lists)
	   (lset-diff+intersection elt=::procedure lis1::pair-nil . lists)
	   (lset-diff+intersection! elt=::procedure lis1::pair-nil . lists))
...
;(define (check-arg pred val caller)
;   (let lp ((val val))
;      (if (pred val) val (lp (error "srfi-1" val caller)))))

;(define-macro (ol-optional arg def)
;   `(if (pair? ,arg) (car ,arg) ,def))
...
(define (iota count #!optional (start 0) (step 1))
  [assert(start)(number? start)]
  [assert(step)(number? step)]
  [assert(count)(>=fx count 0)]
  (let ((last-val (+ start (* (-fx count 1) step))))
    (do ((count count (-fx count 1))
	 (val last-val (-fx val step))
	 (ans '() (cons val ans)))
	((<= count 0)  ans))))
...
;(define (%parse-iota-args arg1::integer rest-args proc)
...
;(define (:iota arg1 . rest-args)
;  (receive (from to step) (%parse-iota-args arg1 rest-args :iota)
;    (let* ((numsteps (ceiling (/ (-fx to from) step)))
;	   (last-val (+ from (* step (-fx numsteps 1)))))
;      (if (< numsteps 0) (error "Negative step count" :iota from to step))
;      (do ((steps-left numsteps (-fx steps-left 1))
;	   (val last-val (-fx val step))
;	   (ans '() (cons val ans)))
...
(define (take lis k::int)
  (let recur ((lis lis) (k k))
    (if (zero? k) '()
	(cons (car lis)
	      (recur (cdr lis) (-fx k 1))))))

(define (drop lis k::int)
  (let iter ((lis lis) (k k))
    (if (zero? k) lis (iter (cdr lis) (-fx k 1)))))
...
(define (take-right lis k::int)
  (let lp ((lag lis)  (lead (drop lis k)))
    (if (pair? lead)
	(lp (cdr lag) (cdr lead))
	lag)))

(define (drop-right lis k::int)
...
(define (drop! lis k::int)
  (if (negative? k)
	...
	(if (zero? nelts) '()
	    (begin (set-cdr! (list-tail lis (-fx nelts 1)) '())
		   lis)))
      (list-tail lis k)))

(define (split-at x k::int)
  (let recur ((lis x) (k k))
...
(define (unfold-right p f g seed #!optional (tail::pair-nil '()))
  (let lp ((seed seed)
	   (ans tail))
    (if (p seed) ans
	(lp (g seed)
...
(define (append-map f::procedure lis1 . lists)
...
;;; Extended from R4RS to take an optional comparison argument.
(define (assoc x lis  #!optional (elt=::procedure equal?))
...
(define (alist-delete key alist #!optional (elt=::procedure equal?))
...
(define (%lset2<= elt= lis1 lis2) (every (lambda (x) (member x lis2 elt=)) lis1))

(define (lset<= elt=::procedure . lists)
  (or (not (pair? lists)) ; 0-ary case
      (let lp ((s1 (car lists)) (rest (cdr lists)))
	(or (not (pair? rest))
	    (let ((s2 (car rest))  (rest (cdr rest)))
	      (and (or (eq? s2 s1)	; Fast path
		       (%lset2<= elt= s1 s2)) ; Real test
		   (lp s2 rest)))))))
	...
	(or (not (pair? rest))
	    (let ((s2   (car rest))
	      ...
	      (and (or (eq? s1 s2)	; Fast path
		       (and (%lset2<= elt= s1 s2) (%lset2<= elt= s2 s1))) ; Real test
		   (lp s2 rest)))))))


(define (lset-adjoin elt=::procedure lis . elts)
  (fold (lambda (elt ans) (if (member elt ans elt=) ans (cons elt ans)))
	lis elts))


(define (lset-union elt=::procedure . lists)
  (reduce (lambda (lis ans)		; Compute ANS + LIS.
	    (cond ((null? lis) ans)	; Don't copy any lists
		  ((null? ans) lis) 	; if we don't have to.
		  ((eq? lis ans) ans)
		  (else
		   (fold (lambda (elt ans) (if (any (lambda (x) (elt= x elt)) ans)
					       ans
					       (cons elt ans)))
			 ans lis))))
	  '() lists))
...
(define (lset-intersection elt=::procedure lis1 . lists)
  (let ((lists (delete lis1 lists eq?))) ; Throw out any LIS1 vals.
    (cond ((any null-list? lists) '())		; Short cut
	  ((null? lists)          lis1)		; Short cut
	  (else (filter (lambda (x)
			  (every (lambda (lis) (member x lis elt=)) lists))
			lis1)))))
...
(define (lset-xor elt=::procedure . lists)
  (reduce (lambda (b a)			; Compute A xor B:
	    ...
	    (receive (a-b a-int-b)   (lset-diff+intersection elt= a b)
	      (cond ((null? a-b)     (lset-difference elt= b a))
		    ((null? a-int-b) (append b a))
		    (else (fold (lambda (xb ans)
				  (if (member xb a-int-b elt=) ans (cons xb ans)))
				a-b
				b)))))
	  '() lists))
	    ...
	    (receive (a-b a-int-b)   (lset-diff+intersection! elt= a b)
	      (cond ((null? a-b)     (lset-difference! elt= b a))
		    ...
		    (else (pair-fold (lambda (b-pair ans)
				       (if (member (car b-pair) a-int-b elt=) ans
...
(define (lset-diff+intersection elt=::procedure lis1 . lists)
  (cond ((every null-list? lists) (values lis1 '()))	; Short cut
	((memq lis1 lists)        (values '() lis1))	; Short cut
	(else (partition (lambda (elt)
			   (not (any (lambda (lis) (member elt lis elt=))
				     lists)))
			 lis1))))
